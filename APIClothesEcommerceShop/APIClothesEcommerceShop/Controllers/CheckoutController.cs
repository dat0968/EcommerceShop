    ...
    public class CheckoutController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetDiscountCoupon(int maUser, string couponcode, decimal originalPrice)
        {
            try
            {
                ...
                var check = await maCouponRepository.CheckUser_CouponCode(maUser, couponcode);
                if (check == true)
                {
                    return Ok(new { Success = false, Message = "Bạn đã sử dụng mã này rồi" });
                }
                var afterApplyCouponPrice = (decimal)(findCouponCode.PhanTramGiam != null && findCouponCode.PhanTramGiam > 0 ? (originalPrice * findCouponCode.PhanTramGiam / 100) : (findCouponCode.SoTienGiam != null && findCouponCode.SoTienGiam > 0 ? findCouponCode.SoTienGiam : 0));
                return Ok(new
                {
                    Success = true,
