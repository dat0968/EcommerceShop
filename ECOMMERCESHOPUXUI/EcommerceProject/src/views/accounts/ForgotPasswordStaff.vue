...
// Xác minh reCAPTCHA với timeout
...
// Tải script khi component được mounted
onMounted(() => {
  loadRecaptchaScript();
...
// Xóa script khi component bị unmounted
onUnmounted(() => {
...
const handleVerifyCode = async () => {
  ...
  try {
    ...
    if (data.success) {
      ...
      router.push({ name: 'ResetPasswordStaff', query: { email: email.value } });
      // Reset reCAPTCHA sau khi thành công
      recaptchaToken.value = '';
      isRecaptchaVerified.value = false;
      window.grecaptcha?.reset();
                  ...
                  <div class="p-3">
                    <form @submit.prevent="showVerification ? handleVerifyCode() : handleForgotPassword()">
                      <div class="text-center mb-3" >
                        <h4 class="text-black"     style="margin-bottom: 10px;">Quên mật khẩu</h4>
                        <p class="text-muted">
