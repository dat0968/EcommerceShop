...
const combo = ref({
  ...
  moTa: '',
  isActive: true,
  ngayBatDau: '', // Thêm trường ngày bắt đầu
  ngayKetThuc: '', // Thêm trường ngày kết thúc
  chitietcombos: [
    {
      maSp: '',
      soLuongSp: 1,
...
const productMap = ref({}) // Ánh xạ maSp -> tenSanPham để hiển thị tên sản phẩm đã chọn
const search = ref('')
// Kích hoạt/ẩn modal Bootstrap khi showProductModal thay đổi
watch(showProductModal, (newValue) => {
  ...
  nextTick(() => {
    ...
    if (modalElement) {
      ...
      console.log('Tìm thấy modal, hành động:', newValue ? 'hiển thị' : 'ẩn')
    } else {
...
async function fetchProducts(page) {
  try {
    ...
    const response = await fetch(
      `${getApiUrl}/api/Products?search=${search.value}&page=${page}`,
      {
        method: 'GET',
        headers: {
...
function openProductModal(index) {
  console.log('Gọi openProductModal', index)
  selectedDetailIndex.value = index
  showProductModal.value = true
  console.log('showProductModal được đặt thành:', showProductModal.value)
...
function selectProduct(product) {
  if (selectedDetailIndex.value !== null) {
    ...
    console.log('Sản phẩm được chọn:', product.maSp, product.tenSanPham)
  }
  ...
  (newcombo) => {
    ...
    if (newcombo.ngayBatDau && newcombo.ngayKetThuc) {
      ...
      if (startDate > endDate) {
        ...
        newcombo.ngayKetThuc = '' // Xóa ngày kết thúc nếu không hợp lệ
      }
    }
  },
  { deep: true }
...
const addCombo = async () => {
  ...
  try {
    ...
    //   const inputValueCombo = element.querySelector('.form-control, .form-select')
    ...
    formData.append('ngayBatDau', combo.value.ngayBatDau) // Thêm ngày bắt đầu
    formData.append('ngayKetThuc', combo.value.ngayKetThuc) // Thêm ngày kết thúc
