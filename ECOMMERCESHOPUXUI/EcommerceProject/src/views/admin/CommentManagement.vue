<template>
  <div class="container-fluid mt-5">
    <nav class="mb-3" style="display: flex; justify-content: center; padding: 10px 0; background-color: transparent;">
    ...
    <div v-else class="row">
        ...
        <div class="card shadow-sm mb-3">
          ...
          <div class="card-body">
            ...
            <div class="form-check">
              <input class="form-check-input" type="radio" name="statusFilter" id="filterPending" value="pending" v-model="filterStatus">
              <label class="form-check-label" for="filterPending">Chờ duyệt</label>
            </div>
            <div class="form-check">
              <input class="form-check-input" type="radio" name="statusFilter" id="filterApproved" value="approved" v-model="filterStatus">
              <label class="form-check-label" for="filterApproved">Đã duyệt</label>
            </div>
            <div class="form-check">
              <input class="form-check-input" type="radio" name="statusFilter" id="filterRejected" value="rejected" v-model="filterStatus">
              <label class="form-check-label" for="filterRejected">Đã từ chối</label>
            </div>
             <div class="form-check">
              <input class="form-check-input" type="radio" name="statusFilter" id="filterAll" value="all" v-model="filterStatus">
      ...
      <div class="col-md-9">
        <div class="card">
          ...
          <div class="card-body">
            <table id="comments-datatable" class="table table-bordered table-striped" style="width: 100%"></table>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="showReasonModal" class="modal fade show" style="display: block; background-color: rgba(0,0,0,0.5);" tabindex="-1">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <p v-if="selectedComment">Bình luận của: <strong>{{ selectedComment.hoTen }}</strong></p>
            <textarea v-model="rejectionReason" class="form-control" rows="3" placeholder="Nhập lý do..."></textarea>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" @click="showReasonModal = false">Hủy</button>
  ...
  methods: {
    async fetchComments() {
      ...
      try {
        ...
        if (response.success) {
          this.allComments = response.data.sort((a, b) => new Date(b.ngayBinhLuan) - new Date(a.ngayBinhLuan));
      ...
      this.$nextTick(() => {
        ...
        this.datatable = $('#comments-datatable').DataTable({
          ...
          columns: [
            ...
            { data: 'ngayBinhLuan', title: 'Ngày bình luận', render: data => new Date(data).toLocaleString() },
            {
              data: 'trangThai',
              title: 'Trạng thái',
              render: (data, type, row) => {
                if (data) {
                  ...
                  return row.lyDoHuy ? `<span class="badge bg-danger" title="Lý do: ${row.lyDoHuy}">Đã từ chối</span>` : '<span class="badge bg-warning text-dark">Chờ duyệt</span>';
                }
              }
            },
            {
              ...
              render: (data, type, row) => {
                if (!row.trangThai && !row.lyDoHuy) { // Only show for pending comments
        ...
        $('#comments-datatable tbody').off('click').on('click', 'button', function () {
    ...
    async approveComment(commentId) {
      try {
        const response = await putToApi(`/staff-comments/${commentId}`, { trangThai: true, lyDoHuy: null });
        if(response.success) {
          Swal.fire({ icon: 'success', title: 'Thành công!', text: 'Bình luận đã được phê duyệt.', timer: 1500, showConfirmButton: false });
          await this.fetchComments();
        } else {
    ...
    async rejectComment() {
      ...
      try {
        const response = await putToApi(`/staff-comments/${this.selectedComment.id}`, { trangThai: false, lyDoHuy: this.rejectionReason });
        if(response.success) {
          Swal.fire({ icon: 'success', title: 'Thành công!', text: 'Bình luận đã bị từ chối.', timer: 1500, showConfirmButton: false });
