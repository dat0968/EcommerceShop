    ...
    <main class="table" id="staff_table" @click.self="cancelDeleteMultiple">
        <section class="table__header">
            ...
            <div class="action-buttons">
                <button class="btn delete-multiple-button" @click="toggleDeleteMultiple">
                    ...
                    {{isDeleteMultipleMode ? `Xác nhận xóa (${staff.filter(s => s.isSelected).length})` : 'Xóa nhiều'
                    }}
                </button>
        ...
        <section class="table__body" style="margin-top: 50px;">
            <table>
                <thead>
                    <tr>
                        ...
                        <th @click="sortTable('hoTen')">Họ Tên <span class="icon-arrow"
                                :class="{ 'active': sortColumn === 'hoTen', 'asc': !sortAsc && sortColumn === 'hoTen' }">↑</span>
                        </th>
                ...
                <tbody>
                    <tr v-for="(staffMember, index) in displayedStaff" :key="staffMember.maNV"
                        ...
                        <td>
                            ...
                            <span class="staff-name" @click="showStaffDetail(staffMember)">{{ staffMember.hoTen
                                }}</span>
                        </td>
                    ...
                    <tr v-if="displayedStaff.length === 0">
                        <td :colspan="isDeleteMultipleMode ? 9 : 8" class="no-data">Không có dữ liệu nhân viên</td>
                    </tr>
                </tbody>
            </table>
        </section>
        ...
        <div class="pagination-container">
            ...
            <div class="pagination">
                ...
                <button @click="changePage(currentPage + 1)" :disabled="currentPage === totalPages"
                    class="pagination-button" title="Trang sau">
                    ›
                </button>
        ...
        <div class="modal" v-if="showDetailModal" @click.self="closeDetailModal">
            <div class="modal-container" style="height: 950px; width: 900px;">
                ...
                <header class="modal-header" id="modal-title"
                    style="font-family: Arial, Helvetica, sans-serif; color: black;">Thông Tin Nhân Viên</header>
                <div class="form-columns">
                    <!-- Left column -->
                    <div class="left-column">
                        <!-- Hình đại diện -->
                        <div class="flex flex-col items-center sm:items-start">
                            <label class="section-title w-full" for="avatar" style="margin-right: 10px;">Hình đại
                                diện</label>
                            <img :src="getImageUrl(selectedStaff.hinh)" alt="Hình đại diện" class="image-preview mt-6"
                ...
                <div class="buttons">
                    ...
                    <button type="button" class="btn-submit" @click="editStaff(selectedStaff.maNV)" aria-label="Sửa"
                        tabindex="0">
                        Sửa
                    </button>
                </div>
    ...
    setup(props, { emit }) {
        ...
        const fetchStaff = async () => {
            try {
                ...
                if (totalCount) {
                    ...
                    try {
                        const countResponse = await axios.get(`${apiUrl}/api/Staff/count`, {
                            params: {
                                hoTen: filterHoTen.value,
                                gioiTinh: filterGioiTinh.value,
                                tinhTrang: filterTinhTrang.value,
        ...
        const getImageUrl = (relativePath) => {
            ...
            if (relativePath.includes('AnhNhanVien')) {
                ...
                return `${apiUrl}/api/Staff/image/${fileName}`;
            }
        ...
        const getStatusClass = (status) => {
            ...
            if (status.includes('tạm khóa') || status.includes('nghỉ việc') || status.includes('khóa')) return 'cancelled';
        ...
        const exportToServer = (fileType) => {
            let url = '';
            if (fileType === 'pdf') {
                url = `${apiUrl}/api/Staff/export/pdf`;
            } else if (fileType === 'excel') {
                url = `${apiUrl}/api/Staff/export/excel`;
            }
                ...
                if (dialogResult.isConfirmed) {
                    try {
                        ...
                        const deletePromises = selectedStaff.map(staffMember =>
                            axios.delete(`${apiUrl}/api/Staff/${staffMember.maNV}`)
                        );
                        ...
                        setTimeout(() => {
                            if (failed > 0) {
                                Swal.fire({
                                    title: 'Cảnh báo',
                                    text: `${failed} nhân viên không thể xóa. ${selectedStaff.length - failed} nhân viên đã xóa thành công.`,
                                    icon: 'warning',
                                    confirmButtonColor: '#f44336',
                                    backdrop: `rgba(0,0,0,0.7)`,
                                    customClass: {
